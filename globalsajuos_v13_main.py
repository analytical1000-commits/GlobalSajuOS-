...
async def rs(kr,modules):
 def _s():
  ...
  if "재물운" in modules:results.append(ModuleResult("재물운","재물운","재물기운","중립",0.55,"운세참고용"))
  return _sx.analyze(results,list(set(r.topic for r in results)))
 try:return await asyncio.to_thread(_s)
...
async def gs(er,uq,modules,cid):
 kj=er.get("kernel",{})
 sj=er.get("synergy",{})
...
@app.get("/api/health")
async def hl():
 return {"status":"ok","version":"v13.0","kernel":"connected","synergy":"connected"}
